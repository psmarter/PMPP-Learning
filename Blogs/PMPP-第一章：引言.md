...
那个年代很简单——提高CPU频率就能让程序跑得更快。摩尔定律很直观：晶体管数量每18个月翻倍，频率提升，性能自然增长。程序员不用操心性能，等两年换台新机器，同样的代码就更快了。

### 功耗墙的出现

2004年前后遇到了物理极限——功耗墙（Power Wall）：

- 动态功耗正比于频率和电压的平方
- 提高频率需要提高电压保证稳定性
- 功耗以超线性速度增长，散热成为瓶颈

当时Pentium 4的TDP已经130W+，如果继续提升频率，功耗会达到数百瓦，在消费级产品中不现实。
...
如果程序只有几百个线程，在GPU上可能不如CPU。GPU需要数千甚至数百万线程才能充分利用硬件。
