...
第十章学了归约——把 N 个数变成 1 个数。这章学**前缀和（Prefix Sum）**，也叫 **Scan**——把 N 个数变成 N 个数，每个位置存储之前所有元素的累计结果。看起来计算量更大，但前缀和是并行计算的"瑞士军刀"，能解决很多看似不可并行的问题：流压缩、基数排序、稀疏矩阵运算等。第十一章重点讲解**工作效率**——如何让并行算法做的总工作量不超过串行算法太多。
...
如果 Section 数量也很大，对 S 的扫描本身需要分层。形成**递归结构**：
...
- [NVIDIA CUB Library - Scan](https://nvlabs.github.io/cub/structcub_1_1_device_scan.html)
